# ==============================================================================
# SA Credit Risk Model Comparison — Full Pipeline
# ==============================================================================
#
# Fits 8 models to the SA simulated credit dataset:
#
...
# ==============================================================================
set.seed(99)
DATA_PATH   <- "C:/Users/23108/Downloads/sa_simulation_17june22h35.csv"   # update if needed
...
# Target: 1 = default (bad), 0 = no default (good)
df_raw$target <- ifelse(df_raw$Risk == "bad", 1L, 0L)
df_raw$Risk   <- NULL
...
# IMPORTANT: "Saving accounts" and "Checking account" contain the string "NA"
# (meaning no formal account), which R reads as a genuine character value but
# model.matrix() interprets as a missing value and silently drops those rows.
# The fix: convert all categorical columns to factors BEFORE encoding, which
# forces "NA" to be treated as a legitimate factor level named "NA".
for (col in CAT_COLS) {
  df_raw[[col]] <- factor(df_raw[[col]], exclude = NULL)    # exclude=NULL keeps NA levels
  # If any true R NA crept in (read_csv artefact), replace with the string "NA"
