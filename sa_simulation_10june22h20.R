...
INPUT_PATH  <- "C:/Users/23108/Downloads/german_cred_data.csv"
OUTPUT_PATH <- "sa_simulation_17june22h35.csv"
...
# ------------------------------------------------------------------------------
# The additive terms above form a purely LINEAR risk structure, which exactly
# matches the functional form of logistic regression / LDA. A simulation built
# only from linear terms structurally favours linear models and biases a
# classical-vs-ML comparison toward the classical side, regardless of what is
# true in real credit data. To make the comparison fair, the terms below add
# interaction effects, a non-monotone (U-shaped) effect, and a ratio-based
# threshold effect — structure that tree-based models (RF, XGBoost) can capture
# but linear models cannot. Each term is grounded in credit-risk reasoning.

# 1) INTERACTION: young AND low/no savings — compounding fragility beyond the
#    sum of the two individual effects (thin-file young borrowers with no buffer)
young_flag    <- age < 28
low_savings   <- saving_accounts %in% c("NA", "little")
default_score <- default_score + ifelse(young_flag & low_savings, 0.35, 0.00)

# 2) INTERACTION: large loan AND informal/unemployed job — affordability risk
#    that only bites when both conditions hold
big_loan      <- credit_amount > 50000
weak_job      <- job %in% c(0, 1)
default_score <- default_score + ifelse(big_loan & weak_job, 0.30, 0.00)

# 3) NON-MONOTONE (U-shaped) duration effect: very short (<6 months) AND very
#    long (>48 months) terms are both riskier than mid-range terms. A linear
#    coefficient cannot represent a U-shape; a tree can.
default_score <- default_score + ifelse(duration < 6 | duration > 48, 0.20, 0.00)

# 4) RATIO / THRESHOLD: loan burden relative to an income proxy implied by job
#    tier. Risk jumps once the loan exceeds a fraction of annual income.
job_income    <- c(15000, 25000, 40000, 70000)[job + 1]   # monthly income proxy by job tier
loan_burden   <- credit_amount / (job_income * 12)         # loan / annual income proxy
default_score <- default_score + ifelse(loan_burden > 0.50, 0.25,
                                  ifelse(loan_burden > 0.25, 0.10, 0.00))

# 5) INTERACTION: informal/free housing AND no checking account — instability
default_score <- default_score + ifelse(housing == "free" & checking_account == "NA", 0.20, 0.00)

# Convert to probability via sigmoid.
#
# SCORE_MULTIPLIER controls how strongly the features predict default (the
# signal-to-noise ratio). With the nonlinear/interaction terms above, a
# multiplier of 3.5 yields an achievable AUC of ~0.74-0.76 and, crucially,
# lets the tree-based models (RF, XGBoost) outperform the linear models by a
# realistic margin (~0.02 AUC) — matching the credit-risk literature
# (Mathibela & Maposa, 2026, best AUC 0.761) rather than being predetermined
# by a linear data-generating process.
#
# INTERCEPT is solved so the mean default probability equals 0.35 at this
# multiplier (NCR Q1 2025: 36.04% impaired records). The two are tuned together.
SCORE_MULTIPLIER <- 3.5
intercept        <- -2.501
logit <- intercept + default_score * SCORE_MULTIPLIER
